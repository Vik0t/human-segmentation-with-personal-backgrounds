...
class EmployeeDataModule:
    ...
    def get_data_for_rendering(self) -> Dict[str, Any]:
        ...
        if self.current_privacy_level == PrivacyLevel.LOW:
            ...
            # Генерируем QR-коды для высокого уровня конфиденциальности
            qr_data = self._generate_qr_codes()
            if qr_data:
    ...
    def _generate_qr_codes(self) -> Optional[QRCodeData]:
        """
        Генерирует QR-коды для контактной информации
        """
        if not self.current_data or not QRCODE_AVAILABLE:
            return None
        ...
        if emp.contact.telegram:
            ...
            telegram_img = telegram_qr.make_image(fill_color="black", back_color="white")
            buffered = BytesIO()
            telegram_img.save(buffered, format="PNG")
            qr_data.telegram_qr_base64 = base64.b64encode(buffered.getvalue()).decode()

        # Генерация QR-кода для полной контактной информации (vCard)
        contact_text = self._generate_vcard_text()
        if contact_text:
            qr_data.contact_text = contact_text
            ...
            contact_img = contact_qr.make_image(fill_color="black", back_color="white")
            buffered = BytesIO()
            contact_img.save(buffered, format="PNG")
    ...
    def generate_contact_qr_data(self) -> Optional[Dict[str, Any]]:
        ...
        result = {
            'contact_text': qr_data.contact_text,
